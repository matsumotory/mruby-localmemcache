...
ht_hash_entry_t *ht_lookup(void *base, va_ht_hash_t va_ht, const char *key, size_t n_key)
{
...
const char *ht_get(void *base, va_ht_hash_t va_ht, const char *key, size_t n_key, size_t *n_value)
{
...
int ht_redo(void *base, va_ht_hash_t va_ht, lmc_log_descriptor_t *l, lmc_error_t *e)
{
  if (l->op_id == LMC_OP_HT_SET) {
    ...
    ht_set(base, va_ht, lmc_string_data(k), lmc_string_len(k), lmc_string_data(v), lmc_string_len(v), e);
    return 1;
  }
  return 0;
}

int ht_set(void *base, va_ht_hash_t va_ht, const char *key, size_t n_key, const char *value, size_t n_value,
           lmc_error_t *e)
{
  ht_hash_t *ht = base + va_ht;
...
int ht_hash_iterate(void *base, va_ht_hash_t va_ht, void *ctx, ht_iter_status_t *s, LMC_ITERATOR_P(iter))
{
  va_ht_hash_entry_t va_hr;
  ht_hash_entry_t *hr = &lmc_null_node;
  ht_hash_t *ht = base + va_ht;
...
int ht_random_pair(void *base, va_ht_hash_t va_ht, char **r_key, size_t *n_key, char **r_value, size_t *n_value)
{
  va_ht_hash_entry_t va_hr;
  ht_hash_t *ht = base + va_ht;
  ht_hash_entry_t *hr = &lmc_null_node;
  ...
  int pair_counter = 0;
  hr = &lmc_null_node;
  for (va_hr = ht->va_buckets[rk]; va_hr != 0 && hr != NULL; va_hr = hr->va_next) {
    hr = va_hr ? base + va_hr : 0;
    pair_counter++;
  }
...
int ht_check_memory(void *base, va_ht_hash_t va_ht)
{
  ...
  ht_hash_entry_t *hr = &lmc_null_node;
  ht_hash_t *ht = base + va_ht;
  if (!lmc_um_mark_allocated(base, bf, va_ht))
    goto failed;
  size_t k;
  for (k = 0; k < LMC_HT_BUCKETS; k++) {
    for (va_hr = ht->va_buckets[k]; va_hr != 0 && hr != NULL; va_hr = hr->va_next) {
      ...
      if (!hr)
        goto next_bucket;
      if (!(lmc_um_mark_allocated(base, bf, va_hr) && lmc_um_mark_allocated(base, bf, hr->va_key) &&
