...
{
  char cs[1024];
  if (lmc_is_filename(s)) {
    lmc_clean_string(cs, s);
  } else {
    strcpy(cs, s);
  }
...
int lmc_lock_obtain(const char *where, lmc_lock_t *l, lmc_error_t *e)
{
  ...
  if (r == -1 && errno == ETIMEDOUT) {
    lmc_handle_error_with_err_string("sem_timedwait", strerror(errno), "LockTimedOut", 0, e);
    return 0;
  }
...
{
  return lmc_handle_error(sem_post(l->sem) == -1, "sem_post", "LockError", l->namespace, e);
}
