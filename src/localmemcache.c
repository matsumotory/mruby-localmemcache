...
int lmc_set_lock_flag(void *base, lmc_error_t *e)
{
  ...
  if (md->locked != 0) {
    lmc_handle_error_with_err_string("lmc_set_lock_flag", "Failed to lock shared memory--may be corrupt!",
                                     "ShmLockFailed", 0, e);
    return 0;
  } else {
    md->locked = 1;
...
int lmc_release_lock_flag(void *base, lmc_error_t *e)
{
  ...
  if (md->locked != 1) {
    lmc_handle_error_with_err_string("lmc_release_lock_flag",
                                     "Shared memory appears to be unlocked already--may be corrupt!", "ShmUnlockFailed",
                                     0, e);
    return 0;
  } else {
    md->locked = 0;
...
int lmc_namespace_or_filename(char *result, const char *ons, const char *ofn, lmc_error_t *e)
{
  if (ons) {
  ...
  lmc_handle_error_with_err_string("lmc_namespace_or_filename", "Need to supply either namespace or filename argument",
                                   "ArgError", 0, e);
  return 0;
}

void lmc_checkize(char *result, char *s)
{
  snprintf(result, 1023, "%s-check", s);
}
...
local_memcache_t *__local_memcache_create(const char *namespace, size_t size, long min_alloc_size, int force, int *ok,
                                          lmc_error_t *e)
{
  int d;
  if (!ok) {
  ...
  *ok = 1;
  ...
  if (!lmc)
    return NULL;
  if ((lmc->namespace = strdup(namespace)) == NULL)
    goto failed;
  lmc->size = size;
  if ((lmc->lock = lmc_lock_init(lmc->namespace, 1, e)) == NULL)
    goto failed;
  int retry_counter = 0;
retry:
  if (retry_counter++ > 10) {
    lmc_handle_error_with_err_string("local_memcache_create", "Too many retries: Failed to repair shared memory!",
                                     "ShmLockFailed", 0, e);
    goto failed;
  }
  if (!lmc_is_lock_working(lmc->lock, e)) {
    if (!force) {
      if (__local_memcache_check_namespace(namespace, e))
        goto retry;
      lmc_handle_error_with_err_string("local_memcache_create", "Failed to repair shared memory!", "ShmLockFailed", 0,
                                       e);
      goto failed;
    }
    *ok = 0;
  }
  {
    ...
    if (!*ok || is_lmc_already_initialized(lmc->base)) {
      ...
      if (lmc_get_db_version(lmc->base) > LMC_DB_VERSION) {
        lmc_handle_error_with_err_string("local_memcache_create", "DB version is incompatible", "DBVersionNotSupported",
                                         0, e);
...
retry:
  ...
  if (!r && (strcmp(lmc->error.error_type, "LockTimedOut") == 0)) {
    if (__local_memcache_check_namespace(lmc->namespace, &lmc->error))
      goto retry;
...
{
  if (lock && !lmc_lock_shm_region("local_memcache_free", lmc))
    return 0;
  int r = ht_hash_destroy(lmc->base, lmc->va_hash);
  if (lock && !lmc_unlock_shm_region("local_memcache_free", lmc))
    return 0;
...
int __local_memcache_check_namespace(const char *clean_ns, lmc_error_t *e)
{
  ...
  if ((check_l = lmc_lock_init(check_lock_name, 1, e)) == NULL) {
    lmc_handle_error_with_err_string("lmc_lock_init", "Unable to initialize lock for checking namespace", "LockError",
                                     check_lock_name, e);
    return 0;
  }
  if (!lmc_lock_obtain_mandatory("local_memcache_check_namespace", check_l, e))
    goto check_lock_failed;
  lmc_mem_descriptor_t *md = 0;
  size_t ns_size = lmc_namespace_size((char *)clean_ns);
  int ok;
  local_memcache_t *lmc = __local_memcache_create((char *)clean_ns, ns_size, 0, 1, &ok, e);
  if (!lmc) {
    lmc_handle_error_with_err_string("__local_memcache_create", "Unable to attach memory pool", "InitError", 0, e);
    goto failed;
  }
  md = lmc->base;
  if (!ok) {
...
unlock_and_release:
  if (md) {
    if (!ht_check_memory(lmc->base, md->va_hash))
      goto failed;
    md->locked = 0;
  }
release : {
...
failed:
  repair_failed = 1;
  lmc_handle_error_with_err_string("local_memcache_check_namespace", "Unable to recover namespace", "RecoveryFailed", 0,
                                   e);
...
{
  int r = 1;
  if (!lmc_release_lock_flag(lmc->base, &lmc->error))
    r = 0;
  lmc_lock_release(who, lmc->lock, &lmc->error);
  return r;
}
...
{
  if (!lmc_lock_shm_region("local_memcache_get", lmc))
    return 0;
...
char *local_memcache_get_new(local_memcache_t *lmc, const char *key, size_t n_key, size_t *n_value)
{
  ...
  if (!lmc_unlock_shm_region("local_memcache_get_new", lmc))
    return 0;
  return new_s;
}
...
{
  if (!lmc_lock_shm_region("local_memcache_random_pair", lmc))
    return 0;
  return ht_random_pair(lmc->base, lmc->va_hash, r_key, n_key, r_value, n_value);
}
...
int local_memcache_random_pair_new(local_memcache_t *lmc, char **r_key, size_t *n_key, char **r_value, size_t *n_value)
{
  ...
  if (!lmc_unlock_shm_region("local_memcache_random_pair", lmc))
    return 0;
  return 1;
}
...
{
  if (!lmc_lock_shm_region("local_memcache_set", lmc))
    return 0;
  int r = ht_set(lmc->base, lmc->va_hash, key, n_key, value, n_value, &lmc->error);
  if (!lmc_unlock_shm_region("local_memcache_set", lmc))
    return 0;
  return r;
}
...
{
  if (!lmc_lock_shm_region("local_memcache_clear", lmc))
    return 0;
  lmc_init_memory(lmc->base, lmc->size);
  lmc_mem_descriptor_t *md = lmc->base;
  int r = 1;
  if ((md->va_hash = ht_hash_create(lmc->base, &lmc->error)) == 0) {
    r = 0;
  } else {
    lmc->va_hash = md->va_hash;
  }
  if (!lmc_unlock_shm_region("local_memcache_clear", lmc))
    return 0;
  return r;
}
...
{
  if (!lmc_lock_shm_region("local_memcache_delete", lmc))
    return 0;
  int r = ht_delete(lmc->base, lmc->va_hash, key, n_key);
  if (!lmc_unlock_shm_region("local_memcache_delete", lmc))
    return 0;
  return r;
}
...
{
  if (!lmc_lock_shm_region("local_memcache_iterate", lmc))
    return 0;
  int r = ht_hash_iterate(lmc->base, lmc->va_hash, ctx, s, iter);
  if (!lmc_unlock_shm_region("local_memcache_iterate", lmc))
    return 0;
  return r;
}
...
{
  lmc_mem_descriptor_t *md = lmc->base;
  if (!lmc_lock_shm_region("local_memcache_check_consistency", lmc))
    return 0;
  int r = ht_check_memory(lmc->base, md->va_hash);
  if (!lmc_unlock_shm_region("local_memcache_check_consistency", lmc))
    return 0;
  return r;
}
