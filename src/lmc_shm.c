...
{
  if (lmc_is_filename(ns)) {
    snprintf(result, 1023, "%s", ns);
  } else {
...
int lmc_clean_namespace(const char *ns, lmc_error_t *e)
{
  ...
  if (lmc_does_namespace_exist(ns)) {
    if (!lmc_handle_error(unlink(fn) == -1, "unlink", "ShmError", fn, e)) {
      return 0;
    }
  }
  return 1;
...
lmc_shm_t *lmc_shm_create(const char *namespace, size_t size, lmc_error_t *e)
{
  ...
  if (!lmc_handle_error((mc->fd = open(fn, O_RDWR, (mode_t)0777)) == -1, "open", "ShmError", fn, e))
    goto open_failed;
  if (!lmc_handle_error(lseek(mc->fd, mc->size - 1, SEEK_SET) == -1, "lseek", "ShmError", fn, e))
    goto failed;
  if (!lmc_handle_error(write(mc->fd, "", 1) != 1, "write", "ShmError", fn, e))
    goto failed;
  mc->base = mmap(0, mc->size, PROT_READ | PROT_WRITE, MAP_SHARED, mc->fd, (off_t)0);
...
{
  int r = lmc_handle_error(munmap(mc->base, mc->size) == -1, "munmap", "ShmError", 0, e);
  close(mc->fd);
  free(mc);
  return r;
}
