...
int lmc_is_va_valid(void *base, size_t va)
{
  ...
  return !(((void *)c < base) || (base + md->total_size + sizeof(lmc_mem_descriptor_t)) < (void *)c);
}
...
{
  lmc_mem_descriptor_t *md = base;
  size_t s = lmc_max(size + sizeof(size_t), sizeof(lmc_mem_chunk_descriptor_t) + sizeof(size_t));
  ...
  size_t r = 0;
  if (c->size - s < sizeof(lmc_mem_chunk_descriptor_t)) {
    s = c->size;
  }
  // -----------------           -------------------
  // | chunk         |   wanted: |                 |
...
void lmc_compact_free_chunks(void *base, size_t va_chunk)
{
  ...
  while (c_free_chunk) {
    ...
    if (va_c_free_chunk != va_chunk) {
      if (c_free_chunk->next == va_chunk) {
        va_chunk_p = va_c_free_chunk;
      } else if (!merge1) {
        // ----------------------
        // | a_free_chunk        |
        // ----------------------   <---- if (...)
        ...
        if (va_c_free_chunk + c_free_chunk->size == va_chunk) {
          ...
          LMC_TEST_CRASH
          if (chunk->next == va_c_free_chunk) {
            va_previous = va_chunk;
          }
          LMC_TEST_CRASH
          lmc_mem_chunk_descriptor_t *p = va_previous ? base + va_previous : 0;
          LMC_TEST_CRASH
          if (p) {
            p->next = c_free_chunk->next;
          }
          LMC_TEST_CRASH
          break;
        }
    ...
    if (va_c_free_chunk == 0) {
      c_free_chunk = NULL;
    } else {
      c_free_chunk = base + va_c_free_chunk;
    }
  }
  // ----------------------
  // | a_free_chunk        |
  ...
  if (merge1) {
    ...
    LMC_TEST_CRASH
    if (md->first_free == va_chunk) {
      md->first_free = chunk->next;
    }
    LMC_TEST_CRASH
    cd->size += c_size;
  }
...
void lmc_free(void *base, size_t chunk)
{
  ...
  if (!(chunk >= sizeof(lmc_mem_descriptor_t) + sizeof(size_t)) || !lmc_is_va_valid(base, chunk)) {
...
int lmc_um_mark(void *base, char *bf, size_t va, size_t size)
{
  size_t i;
  lmc_mem_descriptor_t *md = base;
  if ((va > sizeof(lmc_mem_descriptor_t)) && (!lmc_is_va_valid(base, va) || !lmc_is_va_valid(base, va + size))) {
...
char *lmc_um_new_mem_usage_bitmap(void *base)
{
  ...
  if (!lmc_um_mark(base, bf, 0, sizeof(lmc_mem_descriptor_t)))
    goto failed;
  lmc_mem_chunk_descriptor_t *c;
  while (va) {
    c = base + va;
    if (!lmc_um_mark(base, bf, va, c->size))
      goto failed;
    va = c->next;
  }
  return bf;
